...
/*   Updated: 2019/11/01 00:09:26 by yehuang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			draw_line_j(t_mlx *f, t_line line, int i, int j)
{
	...
	line.y0 = round((int)WIN_L * 0.5 + (f->map[i][j].yf) - f->map[i][j].zf);
	line.x1 = round((int)WIN_W * 0.5 + (f->map[i][j + 1].xf) * f->scale);
	line.y1 = round((int)WIN_L * 0.5 + \
			(f->map[i][j + 1].yf) - f->map[i][j + 1].zf);
...
void			drawline(t_mlx *f)
{
	...
	while (++i < f->row)
	{
		...
		{
			if (j + 1 < f->col)
				draw_line_j(f, line, i, j);
			if (i + 1 < f->row)
				draw_line_i(f, line, i, j);
		}
	}
}
